...
DEFAULT_TERMINATOR = _cfg.get("serial", "DEFAULT_TERMINATOR", fallback="\n")
...
def choose_port_interactively() -> str:
    ...
    if not ports:
        print(
            "No se encontraron puertos serie. Conecta la consola VP2 y vuelve a intentar."
        )
        sys.exit(1)
...
def open_port(
    port: str, baud: int = DEFAULT_BAUD, timeout: float = DEFAULT_TIMEOUT
) -> serial.Serial:
...
def send_command(
    ser: serial.Serial, cmd_bytes: bytes, timeout: float = DEFAULT_TIMEOUT
) -> bytes:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Enviar comando a consola Davis VP2 y mostrar respuesta (interactivo)."
    )
    parser.add_argument(
        "--port",
        "-p",
        help="Puerto serie (ej. COM3). Si no se indica, lista y pide seleccionar.",
    )
    parser.add_argument(
        "--baud",
        "-b",
        ...
        help=f"Baudios (default: {DEFAULT_BAUD})",
    )
    parser.add_argument(
        "--timeout",
        "-t",
        ...
        help=f"Timeout lectura en segundos (default: {DEFAULT_TIMEOUT})",
    )
    parser.add_argument(
        "--terminator",
        default=DEFAULT_TERMINATOR,
        help="Terminador agregado por defecto al comando (default: '\\n')",
    )
    parser.add_argument(
        "--hex",
        action="store_true",
        help="Interpretar la entrada del usuario como bytes hex y enviarlos crudos.",
    )
    parser.add_argument(
        "--raw",
        action="store_true",
        help="Enviar exactamente lo que el usuario escribe (sin añadir terminador).",
    )
    parser.add_argument(
        "--list-ports", action="store_true", help="Listar puertos serie y salir."
    )
    parser.add_argument(
        "--no-wakeup",
        action="store_true",
        help="No ejecutar secuencia de wake-up antes de enviar.",
    )
    parser.add_argument(
        "--show-hex", action="store_true", help="Mostrar respuesta también en hex."
    )
    ...
    try:
        if not args.no_wakeup:
            ...
            if not woke:
                print(
                    "Advertencia: no se detectó respuesta al wake-up (continuando de todos modos)."
                )
